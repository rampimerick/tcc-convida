...
public interface EventRepository extends MongoRepository<Event, String>{
    ...
    @Query("{$and: [{'active': true},{$and: [{$or:[{'dateStart':{$gte: ?0}}, {'dateEnd':{$gte: ?0}}]}, {$or: [{'dateStart':{$lte: ?1}},{'dateEnd':{$lte: ?1}}]}] },{$or:[{'type':{$regex: ?2}},{'type': {$regex: ?3}},{'type': {$regex: ?4}},{'type': {$regex: ?5}},{'type': {$regex: ?6}},{'type': {$regex: ?7}},{'type': {$regex: ?8}}]}, {'name':{ $regex: ?9, $options: 'i'}}]}")
    ...
    @Query("{$and: [{'active': true}, {$and: [{'dateStart':{$lte: ?0}}, {'dateEnd': {$gt: ?1}}]},{$or:[{'type':{$regex: ?2}},{'type': {$regex: ?3}},{'type': {$regex: ?4}},{'type': {$regex: ?5}},{'type': {$regex: ?6}},{'type': {$regex: ?7}},{'type': {$regex: ?8}}]},{'name':{ $regex: ?9, $options: 'i'}}]}")
